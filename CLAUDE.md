...
4. **Winner** — shows the survivor's name. While this phase is up, every *losing* phone applauds in its own enduring, randomly-timed clap pattern (`sfx.clap()` on a self-rescheduling loop); one phone reads as a single clapper, a roomful blends into a steady crowd (the winner's own phone stays quiet). Any player can tap a smiley (💩 / ❤️ / 🕺 / 💃); each tap re-broadcasts a transient `reaction` event that floats one emoji particle up every screen (no counters). After 10 seconds (`winnerEndsAt`) the server returns everyone to the lobby un-readied, but the client keeps the winner on screen and slides the lobby panel up (it fades in) from below — so players can keep emoting. Both the applause and the match soundtrack ride through this transition and end as the lobby panel fades in (held `POSTGAME_HOLD_MS` into the post-game), so neither plays in the idle lobby before the next match. Reactions are accepted by the server in both the `winner` and `lobby` phases for this reason. The post-game winner is cleared on the client when the next round starts.
